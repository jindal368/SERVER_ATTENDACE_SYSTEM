...
export const deleteFaculty = async (req, res) => {
  try {
    ...
    logger.debug(`Req : ${id}`);
    if (
      req.tokenPayload.reason === globalConstants.LOGIN &&
      (await isAdminById(id)) &&
      (await isFacultyVerified(id))
    ) {
      ...
      if (updatedSchema.n == 0)
        res
          ...
          .json({ message: "Faculty does not exists" });
      else
        res
          .status(201)
          .json({ message: "Removed Successfully", updatedSchema });
    } else {
      res.status(400).json({
...
export const forgotPasswordChangePassword = (req, res) => {
  ...
  if (payload.reason === globalConstants.FORGOT_PASSWORD) {
    facultyModal
      ...
      .then((hashedPass) => {
        return facultyModal.findByIdAndUpdate(
          payload.id,
